...
void process_gtpu_packet(upef_t *tunnel, char *buffer, ssize_t len) {
  if (len < (ssize_t)sizeof(struct gtp_header)) {
      LOG_W(UPEF, "GTP packet too short!\n");
      return;
  }
  ...
  if (gtp->message_type == 0xff) { // T-PDU
      ...
      if ((size_t)(payload_len + sizeof(struct gtp_header)) > (size_t)len) {
          LOG_W(UPEF, "Invalid GTP payload length!\n");
          return;
      }
  ...
  while (1) {
      ...
      if (FD_ISSET(tunnel->udp_sock, &read_fds)) {
          ...
          if (len > 0) {
              ...
              LOG_E(UPEF, "UDP recvfrom error: %s\n", strerror(errno));
          }
      }
  }
}
...
int initialize_gtpu_tunnel(upef_t *tunnel, int tunnel_index) {
  ...
  if (tunnel->udp_sock < 0) {
      LOG_E(UPEF, "Failed to create UDP socket: %s\n", strerror(errno));
      return -1;
  }
  ...
  if (bind(tunnel->udp_sock, (struct sockaddr *)&local_addr, sizeof(local_addr)) < 0) {
      LOG_E(UPEF, "Failed to bind UDP socket to %s:%d, error: %s\n",
  ...
  if (tunnel->tun_fd < 0) {
      LOG_E(UPEF, "Failed to create TUN device %s\n", tun_name);
      close(tunnel->udp_sock);
      return -1;
  }
  ...
  int ret = system(cmd);
  if (ret != 0) {
      LOG_E(UPEF, "Failed to assign IP to %s, code: %d\n", tun_name, ret);
      close(tunnel->udp_sock);
      close(tunnel->tun_fd);
      return -1;
  }
  ...
  ret = system(cmd);
  if (ret != 0) {
      LOG_E(UPEF, "Failed to bring up %s, code: %d\n", tun_name, ret);
      close(tunnel->udp_sock);
      close(tunnel->tun_fd);
      return -1;
  }

...
#include <sys/types.h>
#include <netdb.h>
#include <openair3/ocp-gtpu/upef_z-agf1.h>
...
upef_t gtpu_tunnels[MAX_TUNNELS] = {
    {"192.168.60.99", "192.168.60.88", 2153, -1, -1, 0x87654321, PTHREAD_MUTEX_INITIALIZER}
};
//end
...
int create_tun_device(const char *dev) {
    ...
    if (fd < 0) {
        LOG_E(UPEF, "Opening /dev/net/tun failed: %s\n", strerror(errno));
        exit(EXIT_FAILURE);
    }
    ...
    if (ioctl(fd, TUNSETIFF, (void *)&ifr) < 0) {
        LOG_E(UPEF, "TUNSETIFF failed: %s\n", strerror(errno));
        close(fd);
        return -1;
    }

    LOG_I(UPEF, "Created TUN device %s\n", dev);
    return fd;
}
...
void forward_to_gtpu(upef_t *tunnel, char *packet, ssize_t len, uint32_t teid) {
  ...
  LOG_I(UPEF, "Forwarding %zd bytes via GTP-U to %s:%d, TEID: %08x\n",
  ...
  if (sent < 0) {
      LOG_E(UPEF, "Error sending GTP packet: %s\n", strerror(errno));
  } else {
      LOG_I(UPEF, "Sent %zd bytes via GTP-U\n", sent);
  }
}
...
void process_gtpu_packet(upef_t *tunnel, char *buffer, ssize_t len) {
  ...
  if (gtp->message_type == GTP_GPDU) {
      ...
      if (written < 0) {
          LOG_E(UPEF, "Error writing to TUN device: %s\n", strerror(errno));
      } else {
          LOG_I(UPEF, "Wrote %zd bytes to TUN device\n", written);
      }
  } else {
      LOG_W(UPEF, "Unhandled GTP message type: %02x\n", gtp->message_type);
  }
}
...
void gtpu_packet_loop(upef_t *tunnel) {
  ...
  LOG_I(UPEF, "[GTPU] Starting loop for tunnel %s:%d (TEID: %08X)\n",
  ...
  while (1) {
      ...
      if (FD_ISSET(tunnel->tun_fd, &read_fds)) {
          ...
          if (len > 0) {
              LOG_I(UPEF, "Received %zd bytes from TUN device\n", len);
...
int initialize_gtpu_tunnel(upef_t *tunnel, int tunnel_index) {
  ...
  if (tunnel->udp_sock < 0) {
      LOG_E(UPEF, "Failed to create UDP socket\n");
      return -1;
  }
  ...
  if (bind(tunnel->udp_sock, (struct sockaddr *)&local_addr, sizeof(local_addr)) < 0) {
      LOG_E(UPEF, "Failed to bind UDP socket\n");
  ...
  if (tunnel->tun_fd < 0) {
      LOG_E(UPEF, "Failed to create TUN device\n");
      close(tunnel->udp_sock);
      return -1;
  }
  ...
  snprintf(cmd, sizeof(cmd), "ip addr add 10.51.0.%d/24 dev %s", 99 + tunnel_index, tun_name);
  if (system(cmd) != 0) {
      LOG_W(UPEF, "Command '%s' returned non-zero exit status.\n", cmd);
  }
  ...
  LOG_I(UPEF, "Tunnel initialized: %s -> %s (TEID: %08X)\n",
...
int initialize_gtpu_system() {
  for (int i = 0; i < MAX_TUNNELS; i++) {
      if (initialize_gtpu_tunnel(&gtpu_tunnels[i], i) == 0) {
          LOG_I(UPEF, "Tunnel %d setup success\n", i);
      } else {
          LOG_E(UPEF, "Tunnel %d setup failed\n", i);
      }
  }

...
#include <openair3/ocp-gtpu/gtp_itf_z-agf1.h>
                      ...
                      bool npduNumFlag)
{
    ...
    // Salin bearer agar aman sebelum unlock mutex
    gtpv1u_bearer_t tmp = ptr2->second;
    pthread_mutex_unlock(&globGtp.gtp_lock);

    // Inisialisasi GTP-U (sekali saja)
    if (!gtpu_initialized) {
        initialize_gtpu_system();
    }

    // Kirim paket (langsung tanpa loop infinite)
    if (tmp.outgoing_qfi != -1) {
        Gtpv1uExtHeaderT ext = {0};
        ext.ExtHeaderLen = 1;
        ext.pdusession_cntr.spare = 0;
    ...
    LOG_I(GTPU, "[%ld] gtpv1uSendDirect success: UE ID %lx\n", instance, ue_id);
}
...
void *gtpu_packet_loop_thread(void *arg) {
  gtpu_tunnel_t *tunnel = (gtpu_tunnel_t *)arg;
